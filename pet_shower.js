...
(() => {
  ...
  const scale = 1.3; // 130% bigger
  const scaledW = 400 * scale;
  const scaledH = 400 * scale;
  const centeredX = canvas.width / 2 - scaledW / 2; // center horizontally
  const groundedY = groundY - scaledH; // keep feet on ground
  ctx.drawImage(baseImage, centeredX, groundedY, scaledW, scaledH);
}
